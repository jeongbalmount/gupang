...
- 사람이 많이 몰릴 때마다 수동으로 서버 증설을 하기가 쉽지 않은 문제 발생
    - AWS ELB를 도입하여 일정 트래픽 이상 발생하면 새로운 EC2를 생성하여 트래픽 분산

**캐싱 & 세션**

- 상품 리뷰와 같이 많은 양의 데이터를 고객이 부를때마다 쿼리하는 것은 비효율적
    - 그렇기 때문에 Redis repository를 도입하여 쿼리 하지 않고 in-memory 방식으로 빠르게 정보 쿼리 가능하게 구현하였습니다.
    - 세션정보 또한 in-memory 방식으로 빠르게 조회 가능하도록 Redis-session을 도입하였습니다.

**다형성**

- 쿠폰이 퍼센트와 금액 방식으로 나뉘어 있는데 나중에 추가 될 새로운 형식의 쿠폰까지 대비해 쿠폰 도메인을 구성할 필요가 있었습니다.
    - 쿠폰 도메인을 추상 클래스로 설계하여 쿠폰 유효 검사와 같이 모든 쿠폰에 쓰이는 로직은 구현해 놓고 쿠폰 마다 다르게 구현할 할인 방법이나 할인율 등은 추상 메서드로써 상속 받는 클래스에서 구현하도록 구성하였습니다.
    - DB에서 구현하기 힘든 상속 구조를 JPA를 사용하여 DB에서 상속을 사용하는 것처럼 구현하였습니다.
    - DB 구조가 복잡해지지 않게 InheritanceType을 single table 방식을 사용하였습니다.

**예외처리**

- 예외 처리를 보기 좋고 쉽게 처리할 수 있는 방법이 필요
    - @ExceptionHandler 애노테이션을 사용하고, @RestControllerAdvice를 이용해 각 컨트롤러가 갖는 예외를 쉽게 정의하고 정리할 수 있었습니다.
- 필터나 인터셉터에서 발생하는 예외를 어떻게 잡는 문제
    - 따로 global하게 @RestControllerAdvice를 정의하여 각 컨트롤러 뿐만아니라 필터와 인터셉터에서도 예외를 잡을 수 있게 설정했습니다.

**데이터베이스**

- sql의 대부분이 Read 작업인데 crud 모든 작업을 한 db 서버가 맡는 것은 비효율적
    - sql 서버를 master와 slave로 나눠 slave가 read작업을 도맡도록 하고 나머지 update 작업을 master가 맡고 slave와 동기화 할 수 있도록 설정하였습니다.
- jpa는 1대다 혹은 다대1 관계의 엔티티를 지연로딩으로 설정하면 지금 필요한 정보만 쿼리한다는 장점이 있지만 지연로딩 관계 데이터를 쿼리할 때 추가 쿼리가 나가는 N+1문제가 발생.
    - fetch join을 이용해 1대다 혹은 다대1인 엔티티 정보까지 한번에 가져와 추가 쿼리가 없게 리팩토링 하였습니다.
- 1대다 관계에서 fetch join시 페이징할 수 없는 문제 발생
    - `default_batch_fetch_size` 설정을 통해 batch_size로 1쪽 데이터가 다쪽 데이터 만큼 늘어 나지 않아 페이징 가능하게 수정하였습니다.
- Repository의 중복되는 CRUD 코드들이 존재하는 문제 및 동적쿼리를 문자열로 적을때 발생하는 런타임 에러 문제
    - Spring data jpa를 사용해 간단한 CRUD 코드를 줄일 수 있었습니다.
    - 긴 동적쿼리를 작성해도 컴파일시 오류를 잡을 수 있도록 Querydsl을 사용하였습니다.

**로깅**

- 일정시간 이상으로 요청시간이 걸리는 작업에 대해 로그 필요
    - Spring aop를 사용하여 각 컨트롤러 코드를 건드리지 않고 메서드 실행시간을 구할 수 있었습니다. 그렇기 때문에 advice 코드 수정이 필요할 때 aop 클래스만 수정하여 단일 책임원칙을 지켰습니다.
    - 로그는 logback을 사용하였고 logback-spring.xml 파일을 만들어 더 세세한 설정을 가능하게 하고 로그가 log 파일 형태로 남도록 하였습니다.
